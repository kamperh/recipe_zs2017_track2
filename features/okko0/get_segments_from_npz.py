...
"""
Cut segments from a .npz file using a "utterance_start-end" convention.
...
def main():
    ...
    input_npz = dict(np.load(args.input_npz_fn))
    ...
    for target_seg_key in target_segs:
        ...
        output_npz[target_seg_key]  = input_npz[utterance][start:end + 1]
        n_target_segs += 1
        # for utterance_key in [i for i in input_npz.keys() if i.startswith(utterance)]:
        #     print utterance, utterance_key
